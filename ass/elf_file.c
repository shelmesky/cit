...
void Elf_file::addSym(lb_record *lb) {
    ...
    */
    if (name.find("@lab_") == 0 || name.find("@if_") == 0 || name.find("@while_") == 0 || name.find("@cal_") == 0 ||
        name == "@s_stack")
        return;
...
void Elf_file::addSym(string st_name, Elf32_Sym *s) {
    Elf32_Sym *sym = symTab[st_name] = new
    Elf32_Sym();
    if (st_name == "") {
        sym->st_name = 0;
        sym->st_value = 0;
...
void Elf_file::assmObj() {
    ...
    *p_id = 0;
    ...
    //-----添加.rel.text
    curOff += strtabSize;
    addShdr(".rel.text", SHT_REL, 0, 0, curOff, relTextTab.size() * 8, getSegIndex(".symtab"), getSegIndex(".text"), 1,
            8);//.rel.text
    //-----添加.rel.data
    curOff += relTextTab.size() * 8;
    addShdr(".rel.data", SHT_REL, 0, 0, curOff, relDataTab.size() * 8, getSegIndex(".symtab"), getSegIndex(".data"), 1,
            8);//.rel.data
    //更新段表name
    for (int i = 0; i < shdrNames.size(); ++i) {
