...
char symName[][30] =
        {
                ...
                "",
                "break", "char", "continue", "else", "extern", "if", "in", "int", "out", "return", "string", "void",
                "while"//保留字
                , ""
        };
...
void dec()//tested
{
    ...
    } else if (token == rsv_extern)//外部变量声明
    {
        ...
        nextToken();
        if (!match(semicon)) {
            if (token == rsv_extern || token == rsv_void || token == rsv_int || token == rsv_char ||
                token == rsv_string)//丢失分号
            {
...
void funtail(symbol dec_type, string dec_name) {
    ...
    } else if (token == lbrac)//函数定义
    {
        ...
        genFuntail();
        return;
    } else if (token == ident || token == rsv_if || token == rsv_while || token == rsv_return || token == rsv_break ||
               token == rsv_continue
