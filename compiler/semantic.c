...
void fun_record::flushargs()//将参数写到符号表
{
    ...
    for (int i = argslen - 1; i >= 0; i--) {
        ...
        table.addvar(pRec);
        if (showTab)
            cout << "\t\t\t函数 " << symName[this->type] << " " << this->name << "()\t参数变量<" << symName[pRec->type]
                 << ">(" << pRec->name << ")\t进入-->符号表" << endl;
    }
    flushed = 1;
}
...
void fun_record::poplocalvars(int num)//弹出多个局部变量
{
    ...
    for (int i = 0; i < num; i++)//删除记录
    {
        ...
        if ((*localvars)[localvars->size() - 1]->name[0] != '@') {
            if (showTab)
                cout << "\t\t\t函数 " << symName[this->type] << " " << this->name << "()\t局部变量<"
                     << symName[(*localvars)[localvars->size() - 1]->type] << ">("
                     << (*localvars)[localvars->size() - 1]->name << ")\t退出<--符号表" << endl;
        }
    ...
    if (num == -1)//函数定义结束
    {
        ...
        for (int i = 0; i < argslen; i++) {
            if (showTab)
                cout << "\t\t\t函数 " << symName[this->type] << " " << this->name << "()\t参数变量<"
                     << symName[(*localvars)[i]->type] << ">(" << (*localvars)[i]->name << ")\t退出<--符号表" << endl;
    ...
    for (var_i = var_map.begin(); var_i != var_iend; var_i++) {
        ...
        } else//对extern变量不输出
        {
            if (p_v->type == rsv_string) {
                fprintf(fout, "\t%s times 255 db %d\n", genName("str", null, p_v->name).c_str(),
                        isEx);//字符串变量初始化为全0,长度255B
