 ...
 * 	var_num——复合语句里的变量个数
 */
var_record *genExp(var_record *p_factor1, symbol opp, var_record *p_factor2, int
...
if (showGen)
cout << "生成表达式<" << symName[p_factor1->type] << "> " << symName[opp] << " <" << symName[p_factor2->type]
<< "> => <" << symName[rsl_type] << ">" <<
endl;
 ...
 * 产生赋值表达式的代码
 */
var_record *genAssign(var_record *des, var_record *src, int
...
if (showGen)
cout << "生成赋值语句<" << symName[des->type] << "> = <" << symName[src->type] << ">" <<
endl;
 ...
 */
void genReturn(var_record *ret, int
...
if (showGen)
cout << "生成return <" << symName[ret->type] << "> 语句" <<
endl;
if (ret->localAddr < 0)
fprintf(fout,
...
void genInput(var_record *p_i, int
...
if (showGen)
cout << "生成对象<" << symName[p_i->type] << ">(" << p_i->name << ")的输入操作代码" <<
endl;
fprintf(fout,
"\t;为%s产生输入代码\n", p_i->name.
...
void genOutput(var_record *p_o, int
...
if (showGen)
cout << "生成对象<" << symName[p_o->type] << ">的输出操作代码" <<
endl;
fprintf(fout,
"\tmov ecx,[ebp%d]\n\tmov edx,0\n\tmov dl,[ecx]\n\tsub ecx,edx\n\tmov ebx,1\n\tmov eax,4\n\tint 128\n",
p_o->localAddr);
