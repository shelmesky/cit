...
void Elf_file::readElf(const char *dir) {
    ...
    for (int i = 0; i < ehdr.e_shnum; ++i)//读取段表
    {
        ...
        string
        name(shstrTabData + shdr->sh_name);
    ...
    for (hash_map<string, Elf32_Shdr *, string_hash>::iterator i = shdrTab.begin(); i != shdrTab.end(); ++i)//所有段的重定位项整合
    {
        if (i->first.find(".rel") == 0)//是重定位段
        {
            ...
            for (int j = 0; j < relNum; ++j) {
                ...
                string
                name(strTabData + symList[ELF32_R_SYM(rel->r_info)]->st_name);//获得重定位符号名字
                //使用shdrNames[sh_relTab->sh_info]访问目标段更标准
                relTab.push_back(new
                RelItem(i->first.substr(4), rel, name));//添加重定位项
                if (showLink)
...
Elf_file::~Elf_file() {
    ...
    relTab.clear();
    //清空临时存储数据
    if (shstrtab != NULL)delete[]
    shstrtab;
    if (strtab != NULL)delete[]
    strtab;
    if (elf_dir)
        delete elf_dir;
}
